...
public class CharacterSkeleton
{
    ...
    public Transform set(float[] jt, int[] st, int offset, bool mirrored, bool move)
    {
        ...
        for (int i = 0; i < targetBone.Length; i++)
        {
            ...
            if (trackingSegment.ContainsKey(targetBone[i])) { 
                prev = trackingSegment[targetBone[i]];
            }
            trackingSegment[targetBone[i]] = joint[e] - joint[s];            
            if (prev != trackingSegment[targetBone[i]])
            {
                // trackingSegment[targetBone[i]] = cur;                
        ...
        // Debug.Log(HumanBodyBones.RightHand.ToString()+"===");
        return rigBone[HumanBodyBones.RightHand].transform;
    }
}
